                    ...
                    <tbody>
                        {workers.map(worker => (
                            <tr key={worker.id} style={{ textAlign: 'center' }}>
                                ...
                                <td style={{ padding: '10px', border: '1px solid #ddd' }}>{worker.totalWorkedMinutes ? formatDuration(worker.totalWorkedMinutes) : '0m'}</td>
                                <td style={{ padding: '10px', border: '1px solid #ddd' }}>{formatDuration(worker.totalWorkingMinutes)}</td>
                                <td style={{ padding: '10px', border: '1px solid #ddd' }}>{worker.totalWorkedMinutes ? () => { const missingMinutes = Math.max(0, worker.totalWorkingMinutes - worker.totalWorkedMinutes); formatDuration(missingMinutes) } : '0m'}</td>
                                <td style={{ padding: '10px', border: '1px solid #ddd' }}>{worker.totalWorkedMinutes ? () => { const additionalMinutes = Math.max(0, worker.totalWorkedMinutes - worker.totalWorkingMinutes); formatDuration(additionalMinutes) } : '0m'}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
